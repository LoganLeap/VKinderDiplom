...
from DB import offset, insert_data_users,insert_data_seen_users,  select
...
class VKBot:
    def __init__(self):
        ...
        self.vk = vk_api.VkApi(token=access_token_group)  # АВТОРИЗАЦИЯ СООБЩЕСТВА
    ...
    def name(self, user_id):
        ...
        params = {'access_token': access_token_group,
                  'user_ids': user_id,
                  'v': '5.131'}
    ...
    def find_user(self, user_id):
        ...
        try:
            ...
            self.write_msg(user_id, 'Почемууу?')
    ...
    def get_photo_full(self, user_id):
        """ПОЛУЧЕНИЕ ID ФОТОГРАФИИ"""
        list = self.get_photos_id(access_token_group)
        count = 0
        for i in list:
            count += 1
            if count <= 3:
    ...
    def find_persons(self, user_id, offset):
        self.write_msg(self, user_id, self.found_person_info(offset))
        self.person_id(offset)
        insert_data_seen_users(self.person_id(offset), offset) #offset
        self.get_photos_id(self.person_id(offset))
        self.send_photo_3(user_id, 'Фото Пользователя', offset)
        if self.get_photo_1(self.person_id(offset)) != None:
